...
/* Criando uma lista unica com a soma da lista de itens femininos e itens masculinos */  
const Items = M.concat(F);
/* Criando uma lista unica com a soma da lista de itens femininos e itens masculinos */  


/* Função que adiciona cada item da lista unica "Items"
   no ID do HTML 'itens-colecao-completa' */  
FullCollection = () => {
  var CC = document.getElementById('itens-colecao-completa');
  Items.map((val)=>{
    CC.innerHTML += `
...
/* Fim Função que adiciona cada item da lista unica "Items"
   no ID do HTML 'itens-colecao-completa' */  

FullCollection();


/* Função que verifica a chave "quantidade" de cada item
    e caso essa variavel seja superior que 0 adiciona uma Div 
    do Objeto para visualização */
AtualizarSacola = () => {
...
var CCLinks = document.getElementById('itens-colecao-completa').getElementsByClassName('btn-add-carrinho');


/* Para cada elemento do HTML o seu respectivo botão soma 1 na 
  chave 'quantidade' e chama a função atualizar sacola */
for(var i = 0; i < CCLinks.length; i++) {
  CCLinks[i].addEventListener('click', function(){
...
/* Inicio das funções de slider da coleção completa.html */
let count = 1
...
BtnArrowRight = document.querySelector('.manual-navigation-arrow-right')
BtnArrowLeft = document.querySelector('.manual-navigation-arrow-left')


BtnArrowRight.addEventListener('click', nextImage);
BtnArrowLeft.addEventListener('click', backImage);
...
/* Fim das funções de slider slider da coleção completa.html */